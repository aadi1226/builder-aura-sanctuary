...
function signToken(user: User) {
  return jwt.sign({ sub: user.id, email: user.email }, JWT_SECRET, { expiresIn: TOKEN_TTL_SEC });
}
...
export const requestOtp: RequestHandler = (req, res) => {
  ...
  const code = Math.floor(1000 + Math.random() * 900000).toString().slice(0, 6);
...
export const verifyOtp: RequestHandler = (req, res) => {
  ...
  if (!email || !otp || !record) return res.status(400).json({ error: "Invalid or expired OTP" });
  if (record.expiresAt < Date.now() || record.code !== otp) return res.status(401).json({ error: "Incorrect OTP" });

  store.otps.delete(email.toLowerCase());
  let user = store.users.get(email.toLowerCase());
  if (!user) {
    user = { id: crypto.randomUUID(), email: email.toLowerCase(), name: name || email.split("@")[0], provider: "email" };
...
export const me: RequestHandler = (req, res) => {
  ...
  try {
    ...
    const user = Array.from(store.users.values()).find((u) => u.id === payload.sub);
